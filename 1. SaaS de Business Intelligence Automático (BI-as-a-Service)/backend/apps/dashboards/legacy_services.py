...
class DashboardService:
    ...
    def get_dashboard_data(self, dashboard, options=None):
        ...
        if not df.empty:
            ...
            if date_col:
                # Converter para datetime SEM timezone (naive)
                ...
                if period in ('30d', '90d'):
                    ...
                    # Criar datetime SEM timezone para comparação
    ...
    def _process_financial_simple(self, df):
        """Processar dados financeiros com análise detalhada"""
        try:
            if df.empty:
            ...
            # Detectar coluna de tipo (Receita/Despesa)
            type_col = None
            for col in df.columns:
                if 'tipo' in col.lower() or 'type' in col.lower():
                    type_col = col
                    break
            
            # Calcular receitas e despesas
            if type_col:
                # Usar coluna tipo para separar
                receitas_df = df[df[type_col].str.lower().str.contains('receita|revenue', na=False, case=False)]
                despesas_df = df[df[type_col].str.lower().str.contains('despesa|expense', na=False, case=False)]
                total_revenue = float(receitas_df[value_col].sum())
                total_expenses = float(despesas_df[value_col].sum())
            else:
                # Fallback: positivo = receita, negativo = despesa
            ...
            if date_col and type_col:
                ...
                monthly = df_copy.groupby(['month', type_col])[value_col].sum().unstack(fill_value=0)
                for month in monthly.index:
                    revenue_by_month.append({
                        'month': month,
                        'receitas': float(monthly.loc[month].get('Receita', 0)),
                        'despesas': float(monthly.loc[month].get('Despesa', 0)),
                    })
            ...
            if category_col and type_col:
                despesas_cat = despesas_df.groupby(category_col)[value_col].sum().nlargest(10)
                expenses_by_category = [
                    {'name': str(name), 'value': float(value)}
                    for name, value in despesas_cat.items()
                ]
            
            # Gráfico: Receitas por mês (linha)
            revenue_evolution = []
            if date_col and type_col:
                receitas_monthly = receitas_df.copy()
                receitas_monthly[date_col] = pd.to_datetime(receitas_monthly[date_col], errors='coerce')
                receitas_monthly = receitas_monthly.dropna(subset=[date_col])
                receitas_monthly['month'] = receitas_monthly[date_col].dt.to_period('M').astype(str)
                monthly_revenue = receitas_monthly.groupby('month')[value_col].sum()
                revenue_evolution = [
            ...
            payment_col = None
            for col in df.columns:
                if any(word in col.lower() for word in ['pagamento', 'payment', 'forma']):
                    payment_col = col
                    break
            
            if payment_col:
                payment_stats = df.groupby(payment_col)[value_col].agg(['sum', 'count']).reset_index()
                payment_analysis = [
                    {
                        'method': str(row[payment_col]),
                        'total': float(row['sum']),
            ...
            if desc_col and type_col:
                top_desp = despesas_df.nlargest(5, value_col)[[desc_col, value_col]]
                top_expenses = [
                    {'description': str(row[desc_col]), 'value': float(row[value_col])}
            ...
            return {
                ...
                'charts': {
                    'revenue_vs_expenses': revenue_by_month,
